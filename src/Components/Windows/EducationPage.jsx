...
const Education = ({ bounds, nodeRef, setBounds, hideWindow }) => {
    ...
    useEffect(() => {
        ...
        const updateBounds = (rect) => {
            if (nodeRef.current) {
                ...
                setPosition({ x: (window.innerWidth - rect.width) / 2, y: (window.innerHeight - rect.height - rect.height - 30) / 2 })
            }
        };

        updateBounds(rect);
        window.addEventListener('resize', updateBounds(rect));
        return () => window.removeEventListener('resize', updateBounds(rect));
    }, [nodeRef, setBounds]);
